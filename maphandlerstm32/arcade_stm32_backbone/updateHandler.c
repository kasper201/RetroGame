 ...
 * @return int 0 by default
 */
int updateEnemy(struct Map map[MAP_WIDTH][MAP_HEIGHT], uint8_t x, uint8_t y, struct Player* player)
{
...
{
    printf("called met waardes x:%d y:%d type:%d\n", x, y, type);
    // error checking
    if (map[x][y].type != 0)
        return 1;
    //if (type < 1 || type > 4)
    ...
    printf("info met waardes hp:%d dps:%d type:%d\n", health, damage, type);
...
int createRobot(struct Map map[MAP_WIDTH][MAP_HEIGHT], uint8_t y, uint8_t type, struct Player* player)
{
    ...
    if (type < 5 || type > 7) {
        ...
        if (map[7][y].type != 0) {
            ...
            printf("error 2");
            printf("%d", map[MAP_WIDTH-1][y].type);
            return 1;
        }
        if (y < 0 || y >= MAP_HEIGHT) {
            printf("error 3");
